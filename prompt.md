...
リポジトリを clone した直後に、Hooks 設定とブロックスクリプトが置かれていることを確認します。

```
ls .cortex/settings.json .cortex/hooks/
```

`settings.json` と3本のシェルスクリプト（`block_dangerous_sql.sh` / `block_dangerous_bash.sh` / `log_sql.sh`）が見えていれば OK です。
...
```
.cortex/settings.json と .cortex/hooks/ 配下のスクリプトを読んで、次の3点をそれぞれ1行で説明してください。
...
- 5-2.（オプション）Hook の発火を体感する

時間に余裕があれば、Hooks が実際に書き込み SQL をブロックする様子を確認できます。
...
これらのテーブルの関係をdraw.ioでER図としてhtmlで可視化して
```
